...
player addEventHandler ["Fired",
{
	...
	// Remove explosives in the safe zone
	if (_this select 1 == "Put") then
	{
		...
		if (_ammo isKindOf "TimeBombCore") then // all explosives
		{
			...
			if (_player inArea "safe_zone_marker") exitWith
			{
				deleteVehicle _bomb;
				playSound "FD_CP_Not_Clear_F";
				titleText [format ["You are not allowed to place explosives in the safe zone.\nThe explosive has been removed.", _minDist], "PLAIN DOWN", 0.5];
			};
		};
	} else {
		if (_player inArea "safe_zone_marker") exitWith {
